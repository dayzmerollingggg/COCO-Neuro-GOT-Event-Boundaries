...
# ---------- CONFIG ----------
VIDEO_DIR = '.'  # <<< SET THIS
SAVE_FEATURES_PATH = 'visual_features.npy'
SAVE_PCA_PATH = 'visual_features_pca.npy'

def preprocess_frame(frame):
    """Preprocess a single frame for AlexNet, resizing height first and then center cropping."""
    frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    image = Image.fromarray(frame)

    # Calculate the new width to maintain aspect ratio
    original_width, original_height = image.size
    new_width = int(original_width * (224 / original_height))

    # Resize with height 224 and proportional width
    resized_image = image.resize((new_width, 224), Image.BILINEAR)

    # Calculate crop coordinates for center crop
    crop_left = (new_width - 224) // 2
    crop_right = crop_left + 224
    cropped_image = resized_image.crop((crop_left, 0, crop_right, 224))
    ...
    tensor = transform(cropped_image).unsqueeze(0)  # Shape: (1, 3, 224, 224)
    return tensor

...
def compute_motion_energy(video_file):
    ...
    cap.release()
    print(f"Processed {os.path.basename(video_file)} - Average Motion Energy: {np.mean(motion_energy) if motion_energy else 0:.4f}")
...
def process_videos(directory):
    ...
    for filename in os.listdir(directory):
        if filename.lower().endswith(".mov"):
            ...
            avg_motion_energy = compute_motion_energy(video_path)
